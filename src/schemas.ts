...
export const AreaQuerySchema = z.object({
  region: z.string().optional().describe("地区名（如 北海道地方、関東・甲信地方）"),
...
export const WeatherQuerySchema = z.object({
  code: z.string().regex(/^\d{4,5}$/, "格式: 4-5位地区编码，如 13101").describe("统一地区编码"),
});

export const EarthquakeQuerySchema = z.object({
  code: z.string().regex(/^\d{4,5}$/, "格式: 4-5位地区编码，如 13101").optional().describe("统一地区编码（按都道府県过滤）"),
});
